...
def minimax_move(state, depth_max, eval_func:Callable) -> Tuple[int, int]:
    """
    ...
    move_value, best_move = max_value(state, alpha, beta, 0, depth_max, eval_func, player)
    end_time = time.time()
    
    return best_move

def max_value(state, alpha, beta, depth, depth_max, eval_func, player, parent_state = None, sequencia=0) -> float:
    """
    ...
    if (state_is_terminal or depth == depth_max):
    ...
    for move in legal_moves:
        ...
        if new_state.player == player:
            move_value = max_value(new_state, alpha, beta, depth + 1, depth_max, eval_func, player, state, sequencia + 1)[0]
        else:
            move_value = min_value(new_state, alpha, beta, depth + 1, depth_max, eval_func, player, state, sequencia)
...
def min_value(state, alpha, beta, depth, depth_max, eval_func, player, parent_state, sequencia=0) -> float:
    """
    ...
    for move in legal_moves:
        new_state = state.next_state(move)
        if new_state.player == player:
            move_value, move_return = max_value(new_state, alpha, beta, depth + 1, depth_max, eval_func, player, state, sequencia)
        else:
            move_value = min_value(new_state, alpha, beta, depth + 1, depth_max, eval_func, player, state, sequencia - 1)
        if move_value < beta:
            beta = move_value
            best_move = move
        if beta < alpha:
