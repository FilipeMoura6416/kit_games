    ...
    for j in range(i + 1, len(itens)):
        for h in range(2):
            ...
            for match in range(match_number):
                ...
                try:
                    resultado = subprocess.run(f'start cmd /k "cd /d "{pasta_kit_games}" & python server.py othello {itens[player_b][1]} {itens[player_w][1]} -d 6"', capture_output=True, text=True, shell=True)
                ...
                for agent, content in enumerate(root.findall('player')):
                    agents_scores[agent] += int(content.find('score'))
