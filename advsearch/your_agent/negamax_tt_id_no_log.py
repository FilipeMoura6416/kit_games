...
from .mcts import make_move as mm_mcts
import copy
...
def make_move(state) -> Tuple[int, int]:
    ...
    """
    if state.board.piece_count["W"] + state.board.piece_count["B"] < 48: 
        root_state = Nodo_State(state, None)

        return negamax_move(root_state, 4.7, evaluate_custom)
    else:
        return mm_mcts(state)
...
def negamax(root_state:Nodo_State, eval_func, alpha, beta, depth_max, time_limit:float, my_player, previous_state, type, depth = 0, tt_dict:dict=dict()) -> tuple:
    ...
    for child_node in root_state.children:
        ...
        if time.time() >= time_limit - (0.1 * depth) or move_value == None:
            return None,None
